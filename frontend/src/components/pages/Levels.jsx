...
export default function Levels() {
  ...
  return (
    <>
      <button className="border p-2" onClick={()=>{navigate("/dashboard")}}>Go to Dashboard</button>
          ...
          return (
            <div key={level} className="flex justify-center relative">
              <div className={`w-full flex ${isLeft ? "justify-start pl-10" : "justify-end pr-10"}`}>
                <button className="level relative z-10" onClick={()=>{navigate(`/level/${level}/${category}`);}}>
                  Level {level}
                </button>
              </div>
            </div>
